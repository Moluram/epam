...
/**
 * Represent an implementation of simplex method for maximization
 */
public class SimplexMethodMaximizationSolver implements ModelSolver {
  /**
  ...
  private int findMainCol() {
    ...
    for (int j = 2; j < columnsCount; j++) {
      if (table[rowsCount - 1][j] < table[rowsCount - 1][mainCol]) {
        mainCol = j;
      }
    }
    return mainCol;
  ...
  private int findMainRow(int mainCol) {
    ...
    for (int i = mainRow + 1; i < rowsCount - 1; i++) {
